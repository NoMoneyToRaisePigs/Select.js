...
function initSelect(id,source){
                ...
                if(targetClassList.contains('btn')){
                    if(targetClassList.contains('all')){
                        if(targetClassList.contains('dropdown-select-btn')){
                            ...
                            var btnGroupIndex = event.target.getAttribute('name').split('_')[1];
            ...
            if(btnGroundIndex != undefined){
                dataSource.data[btnGroundIndex].options.forEach(function(optionItem,optionIndex){
                    if(isAdd){
                        ...
                        tryRemoveValue(dataSource.data[btnGroundIndex].options[optionIndex]);
                        optionItem.classList.remove('selected');
                    }                 
                });
            }
            else{
                groups.forEach(function(groupItem, groupIndex){
                    groupItem.options.forEach(function(optionItem, optionIndex){
                        if(isAdd){
                            tryAddValue(dataSource.data[groupIndex].options[optionIndex]);
                            optionItem.classList.add('selected');
    ...
    function setEventsForDropdownSearchInput(){
        _dropdownSearchInput.onkeyup = function(event){
            ...
            _dropdownGroups.forEach(function(groupItem){
                var groupMatchingCounts = groupItem.options.length;
                searchingResultGroup = {group:groupItem.group, options:[]};
                ...
                groupItem.options.forEach(function(optionItem){               
                    if(optionItem.innerText.toLowerCase().indexOf(searchString) != -1){
                        ...
                        searchingResultGroup.options.push(optionItem);
                    }
                    else{
